...
function protectHtml(value) {
  ...
  const policy = "default-src 'self' data: blob:; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'none'; media-src 'self' data: blob:; object-src 'none'; frame-src 'none'; form-action 'none'; base-uri 'self'; frame-ancestors 'none'";
...
async function readPreviewFile(githubCredential, binding, requestedPath) {
  ...
  const data = contentType.startsWith('text/html')
    ...
    : file.data;
  return { path: file.path, contentType, data };
}
