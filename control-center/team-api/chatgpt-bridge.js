...
function canProjectsRead(store, user) {
  return store.capabilities(user).includes('projects.read');
}

function unauthorized(res) {
  json(res, 401, { error: 'invalid_token', error_description: 'Authentication required' }, {
    'WWW-Authenticate': `Bearer error="invalid_token", error_description="Authentication required", resource_metadata="${RESOURCE_METADATA_URL}", scope="projects:read"`
  });
}
...
function protectedResourceMetadata() {
  return {
    resource: RESOURCE,
    resource_name: 'UCHIHA Control Center',
...
function redirectWithCode(res, redirectUri, code, state) {
  ...
  res.writeHead(302, { Location: out.toString(), 'Cache-Control': 'no-store', 'Referrer-Policy': 'no-referrer' });
  res.end();
}
...
function toolList() {
  return [
    {
      name: 'list_projects',
      title: 'List UCHIHA projects',
      description: 'List projects the signed-in UCHIHA member can read. Returns sanitized operational metadata only.',
      inputSchema: { type: 'object', properties: {}, additionalProperties: false },
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }
    },
    {
      name: 'get_project',
      title: 'Get UCHIHA project',
      description: 'Get sanitized status and operational metadata for one UCHIHA project by id.',
      inputSchema: {
        type: 'object',
        properties: { projectId: { type: 'string', minLength: 1, maxLength: 80 } },
        required: ['projectId'],
...
async function handleMcp(req, res) {
  ...
  if (message.jsonrpc !== '2.0' || typeof message.method !== 'string') {
    return json(res, 400, rpcError(id, -32600, 'Invalid Request'));
  }
  ...
  if (message.method === 'initialize') {
    ...
    return json(res, 200, rpcResult(id, {
      ...
      serverInfo: { name: 'uchiha-control-center', version: '2.0.0-alpha17' },
      instructions: 'Read-only UCHIHA project access. Never assume permission to modify or deploy.'
    }));
  }
    ...
    try {
      ...
      if (name === 'get_project') {
        ...
        if (!/^[a-zA-Z0-9._-]{1,80}$/.test(projectId)) {
          return json(res, 200, rpcResult(id, { isError: true, content: [{ type: 'text', text: 'Invalid project id.' }] }));
        }
        const project = registry().get(projectId);
        if (!project) {
          return json(res, 200, rpcResult(id, { isError: true, content: [{ type: 'text', text: 'Project not found.' }] }));
        }
        return json(res, 200, rpcResult(id, {
      ...
      return json(res, 200, rpcResult(id, {
        isError: true,
        content: [{ type: 'text', text: 'UCHIHA project registry is unavailable.' }]
      }));
    }
  }
...
async function handler(req, res, parsedUrl) {
  ...
  if ((pathname === '/.well-known/oauth-protected-resource' || pathname === RESOURCE_METADATA_PATH) && req.method === 'GET') {
  ...
  if (pathname === '/oauth/register' && req.method === 'POST') {
    try {
      return json(res, 201, oauth.registerClient(JSON.parse(await readBody(req) || '{}')));
    } catch (error) {
      return oauthError(res, 400, error.code || 'invalid_client_metadata');
    }
  }
  if (pathname === '/oauth/authorize' && req.method === 'GET') {
    const params = parseAuthorizeParams(url.searchParams);
    try {
      oauth.validateAuthorizationRequest(params, RESOURCE);
      return html(res, 200, authorizePage(params));
    } catch (error) {
      return html(res, 400, authorizePage(params, `تعذر بدء الربط: ${error.code || 'invalid_request'}`));
    }
  }
  if (pathname === '/oauth/authorize' && req.method === 'POST') {
    ...
    try {
      ...
      return html(res, 400, authorizePage({}, 'طلب غير صالح.'));
    }
    const params = parseAuthorizeParams(form);
    let validated;
    try {
      validated = oauth.validateAuthorizationRequest(params, RESOURCE);
    } catch (error) {
      return html(res, 400, authorizePage(params, `طلب OAuth غير صالح: ${error.code || 'invalid_request'}`));
    }
    ...
    if (!auth || !canProjectsRead(auth.store, auth.user)) {
      return html(res, 401, authorizePage(params, 'اسم المستخدم أو كلمة المرور غير صحيحة، أو الحساب لا يملك صلاحية قراءة المشاريع.'));
    }
    clearRate(req);
    ...
    try {
      if (grant === 'authorization_code') {
        const tokens = oauth.exchangeCode({
          ...
          resource: String(form.get('resource') || RESOURCE)
        });
        return json(res, 200, tokens);
      }
      if (grant === 'refresh_token') {
        const tokens = oauth.refresh({
      ...
      return oauthError(res, 400, error.code || 'invalid_grant');
    }
  }
