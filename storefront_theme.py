...
<head>
  ...
  <style>
    :root{
      --bg:#08090d;--bg-2:#0b0c11;--panel:#121218;--panel-2:#19191f;--panel-3:#222229;
      --text:#f7f4f5;--muted:#9b9295;--line:rgba(228,49,63,.13);
      --primary:#e4313f;--secondary:#9f111b;--accent:#d7d9de;
      --primary-rgb:228,49,63;--secondary-rgb:159,17,27;
      --gold:#d7d9de;--danger:#ff6672;--shadow:0 22px 70px rgba(0,0,0,.5);
      --header:74px;--bottom:84px;--radius:22px;--safe-bottom:env(safe-area-inset-bottom,0px)
    }
    *{box-sizing:border-box}
    html{background:var(--bg);scroll-behavior:smooth}
    body{margin:0;min-height:100vh;background:
      radial-gradient(circle at 90% -10%,rgba(var(--primary-rgb),.14),transparent 29%),
      radial-gradient(circle at -10% 38%,rgba(var(--secondary-rgb),.09),transparent 24%),var(--bg);
    ...
    .top-accent{height:4px;position:fixed;inset:0 0 auto;z-index:90;background:linear-gradient(90deg,var(--secondary),var(--primary),var(--accent))}
    .header{position:sticky;top:0;z-index:50;height:var(--header);border-bottom:1px solid var(--line);background:rgba(8,9,13,.9);backdrop-filter:blur(18px)}
    ...
    .hero{position:relative;overflow:hidden;border:1px solid rgba(var(--primary-rgb),.25);border-radius:24px;background:var(--panel);box-shadow:var(--shadow),0 0 55px rgba(var(--primary-rgb),.07);aspect-ratio:15/6;min-height:190px}
    ...
    .slide:after{content:"";position:absolute;inset:0;z-index:-1;background:linear-gradient(90deg,rgba(8,9,13,.08),rgba(8,9,13,.64) 58%,rgba(8,9,13,.95))}
    ...
    .slide-kicker{display:inline-flex;align-items:center;gap:7px;width:max-content;padding:7px 10px;border:1px solid color-mix(in srgb,var(--slide-accent) 38%,transparent);border-radius:99px;background:color-mix(in srgb,var(--slide-accent) 12%,transparent);color:#f3d6d9;font-size:10px;font-weight:900}
    .slide h2{margin:13px 0 8px;font-size:clamp(23px,4vw,43px);line-height:1.22}.slide p{margin:0;color:#cec6c9;line-height:1.75;font-size:13px;max-width:530px}
    .slide-cta{width:max-content;margin-top:17px;padding:10px 17px;border:0;border-radius:13px;background:var(--slide-accent);color:#fff;font-weight:900;cursor:pointer}
    ...
    .ticker{display:flex;align-items:center;gap:11px;margin:14px 0;padding:12px 15px;border:1px solid var(--line);border-radius:16px;background:rgba(18,18,24,.82);color:#d2cacc;font-size:12px;overflow:hidden}
    ...
    .product-img{position:relative;aspect-ratio:1.25/1;overflow:hidden;background:var(--panel-2)}.product-img img{width:100%;height:100%;object-fit:cover}.provider-tag{position:absolute;top:8px;right:8px;padding:5px 7px;border-radius:8px;background:rgba(8,9,13,.78);font-size:8px;color:#ddd7d9}
    .product-body{padding:12px}.product-body h3{min-height:39px;margin:0 0 8px;font-size:12px;line-height:1.62;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.product-meta{display:flex;align-items:center;justify-content:space-between;gap:7px}.price{color:#fff3f4;font-size:14px;font-weight:900}.stock{color:var(--muted);font-size:9px}.buy-mini{width:31px;height:31px;display:grid;place-items:center;border:0;border-radius:10px;background:linear-gradient(135deg,var(--primary),var(--secondary));color:#fff;cursor:pointer}.buy-mini .icon{width:16px;height:16px;stroke-width:2.4}
    .empty,.loading{grid-column:1/-1;padding:38px 18px;border:1px dashed rgba(255,255,255,.12);border-radius:19px;background:rgba(18,18,24,.58);color:var(--muted);text-align:center;line-height:1.8}
    ...
    .wallet-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:11px}.wallet-card{min-height:118px;padding:17px;border:1px solid var(--line);border-radius:20px;background:linear-gradient(145deg,var(--wc1),var(--wc2));position:relative;overflow:hidden}.wallet-card:after{content:"";position:absolute;width:90px;height:90px;left:-25px;bottom:-35px;border:18px solid rgba(255,255,255,.04);border-radius:50%}.wallet-card span{display:block;color:#ddd6d8;font-size:11px}.wallet-card strong{display:block;margin-top:12px;font-size:25px}.wallet-card.primary{--wc1:#641018;--wc2:#181217}.wallet-card.blue{--wc1:#3a151a;--wc2:#17171d}.wallet-card.purple{--wc1:#342a2d;--wc2:#15151a}
    .history{display:grid;gap:9px}.history-row,.order-row,.method-row{display:flex;align-items:center;gap:12px;padding:13px;border:1px solid var(--line);border-radius:16px;background:var(--panel-2)}.history-icon,.method-icon{width:42px;height:42px;display:grid;place-items:center;border-radius:14px;background:rgba(var(--primary-rgb),.08);color:var(--primary);font-weight:900}.grow{flex:1;min-width:0}.grow b{display:block;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.grow small{display:block;margin-top:5px;color:var(--muted);font-size:9px}.amount{font-size:12px;font-weight:900}.amount.minus{color:#ffb2c0}.amount.plus{color:#f1dadd}
    .status{padding:6px 9px;border-radius:99px;background:rgba(var(--secondary-rgb),.12);color:#f0c8cd;font-size:9px;font-weight:900}.status.completed,.status.approved{background:rgba(215,217,222,.1);color:#eef0f3}.status.pending,.status.processing,.status.waiting_payment{background:rgba(215,217,222,.09);color:#d7d9de}.status.cancelled,.status.rejected{background:rgba(255,104,129,.1);color:#ffafbd}
    ...
    .link-card{text-align:center;padding:36px 22px}.link-orb{width:100px;height:100px;display:grid;place-items:center;margin:0 auto 20px;border:1px solid rgba(var(--primary-rgb),.3);border-radius:33px;background:radial-gradient(circle,rgba(var(--primary-rgb),.25),rgba(var(--secondary-rgb),.07));box-shadow:0 0 60px rgba(var(--primary-rgb),.11)}.link-orb img{width:66px}.link-card h2{margin:0 0 9px}.link-card p{max-width:610px;margin:0 auto 20px;color:var(--muted);font-size:12px;line-height:1.9}.linked-badge{display:inline-flex;align-items:center;gap:7px;padding:8px 12px;border-radius:99px;background:rgba(var(--primary-rgb),.1);color:#f3d6d9;font-size:11px;font-weight:900}
    .primary-btn,.outline-btn,.danger-btn{min-height:49px;padding:0 20px;border-radius:15px;font-weight:900;cursor:pointer;transition:.18s}.primary-btn{border:0;background:linear-gradient(135deg,var(--primary),var(--secondary));color:#fff;box-shadow:0 14px 35px rgba(var(--primary-rgb),.16)}.outline-btn{border:1px solid var(--line);background:var(--panel-2);color:var(--text)}.danger-btn{border:1px solid rgba(255,104,129,.28);background:rgba(255,104,129,.06);color:#ff9cae}.wide{width:100%}.primary-btn:active,.outline-btn:active,.danger-btn:active{transform:scale(.98)}
    ...
    .bottom-nav{position:fixed;z-index:55;right:0;left:0;bottom:0;height:calc(var(--bottom) + var(--safe-bottom));padding-bottom:var(--safe-bottom);border-top:1px solid var(--line);background:rgba(8,9,13,.94);backdrop-filter:blur(20px)}.nav-inner{width:min(680px,100%);height:var(--bottom);display:grid;grid-template-columns:repeat(5,1fr);margin:auto}.nav-btn{position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;border:0;background:none;color:#7d7477;font-size:9px;cursor:pointer}.nav-btn .icon{width:22px;height:22px}.nav-btn.active{color:var(--primary)}.nav-btn.home span:first-child{width:54px;height:54px;display:grid;place-items:center;margin-top:-25px;border:1px solid rgba(var(--primary-rgb),.25);border-radius:50%;background:var(--panel);box-shadow:0 0 0 8px rgba(8,9,13,.96),0 8px 30px rgba(0,0,0,.3);color:var(--primary)}.nav-btn.home .icon{width:24px;height:24px}
    .float-tools{position:fixed;z-index:45;left:max(14px,calc((100vw - 1120px)/2 + 10px));bottom:calc(var(--bottom) + var(--safe-bottom) + 18px);display:grid;gap:9px}.float-btn{width:49px;height:49px;display:grid;place-items:center;border:0;border-radius:17px;background:linear-gradient(145deg,var(--primary),var(--secondary));color:#fff;box-shadow:0 12px 30px rgba(var(--primary-rgb),.2);cursor:pointer}.float-btn.muted{background:var(--panel-2);color:var(--text);border:1px solid var(--line)}
    ...
    .drawer{position:fixed;z-index:85;inset:0 0 0 auto;width:min(390px,90vw);display:flex;flex-direction:column;border-left:1px solid var(--line);background:#0d0d12;box-shadow:-30px 0 90px rgba(0,0,0,.5);transform:translateX(103%);transition:.3s cubic-bezier(.22,.75,.2,1)}.drawer.show{transform:none}.drawer-head{padding:25px 20px;border-bottom:1px solid var(--line)}.profile-line{display:flex;align-items:center;gap:13px}.avatar{width:53px;height:53px;display:grid;place-items:center;border:1px solid rgba(var(--primary-rgb),.25);border-radius:18px;background:linear-gradient(145deg,rgba(var(--primary-rgb),.16),rgba(var(--secondary-rgb),.12));font-size:19px;font-weight:900}.profile-line h3{margin:0;font-size:15px}.profile-line p{margin:5px 0 0;color:var(--muted);font-size:10px}.drawer-balance{display:flex;align-items:center;justify-content:space-between;margin-top:15px;padding:12px 14px;border:1px solid rgba(var(--primary-rgb),.15);border-radius:15px;background:rgba(var(--primary-rgb),.055);font-size:12px}.drawer-menu{padding:12px;overflow:auto}.menu-btn{width:100%;height:49px;display:flex;align-items:center;gap:13px;padding:0 14px;border:0;border-radius:14px;background:none;color:#d9d3d5;cursor:pointer;text-align:right}.menu-btn:hover,.menu-btn.active{background:var(--panel-2);color:var(--primary)}.menu-btn .icon{width:20px;height:20px}.menu-btn small{margin-inline-start:auto;color:var(--muted)}.drawer-foot{margin-top:auto;padding:16px;border-top:1px solid var(--line)}
    .modal{position:fixed;z-index:100;right:50%;top:50%;width:min(590px,calc(100% - 24px));max-height:min(88vh,800px);overflow:auto;border:1px solid var(--line);border-radius:24px;background:#111116;box-shadow:var(--shadow);transform:translate(50%,-45%) scale(.96);opacity:0;pointer-events:none;transition:.22s}.modal.show{transform:translate(50%,-50%) scale(1);opacity:1;pointer-events:auto}.modal-head{position:sticky;top:0;z-index:2;display:flex;align-items:center;justify-content:space-between;padding:15px 17px;border-bottom:1px solid var(--line);background:rgba(17,17,22,.95);backdrop-filter:blur(14px)}.modal-head b{font-size:14px}.close-btn{width:38px;height:38px;display:grid;place-items:center;border:1px solid var(--line);border-radius:12px;background:var(--panel-2);cursor:pointer}.modal-body{padding:18px}.product-modal-top{display:grid;grid-template-columns:110px 1fr;gap:15px;align-items:center}.product-modal-img{aspect-ratio:1;border-radius:17px;overflow:hidden;background:var(--panel-2)}.product-modal-img img{width:100%;height:100%;object-fit:cover}.product-modal-top h2{margin:0 0 9px;font-size:19px}.modal-price{display:inline-flex;padding:7px 10px;border-radius:10px;background:rgba(var(--secondary-rgb),.13);color:#f0c8cd;font-weight:900}.product-desc{margin:15px 0;color:var(--muted);font-size:11px;line-height:1.8}.field{margin-bottom:12px}.field label{display:block;margin-bottom:7px;color:#ddd6d8;font-size:11px;font-weight:900}.input,.select,.textarea{width:100%;min-height:50px;padding:0 14px;border:1px solid var(--line);border-radius:15px;background:var(--panel-2);color:var(--text);outline:0}.textarea{min-height:90px;padding-top:13px;resize:vertical}.input:focus,.select:focus,.textarea:focus{border-color:rgba(var(--primary-rgb),.42);box-shadow:0 0 0 4px rgba(var(--primary-rgb),.07)}.buy-actions{display:grid;grid-template-columns:1.2fr .8fr;gap:10px;margin-top:17px}.fine-note{margin-top:15px;padding-right:12px;border-right:3px solid var(--primary);color:var(--muted);font-size:10px;line-height:1.75}
    .auth-screen{position:fixed;z-index:200;inset:0;display:grid;place-items:center start;padding:22px max(22px,calc((100vw - 1120px)/2));background:linear-gradient(90deg,rgba(8,9,13,.12),rgba(8,9,13,.68) 48%,rgba(8,9,13,.97)),url("/assets/hero-madara-v2.webp") center/cover no-repeat;overflow:auto}.auth-card{width:min(560px,100%);padding:31px;border:1px solid rgba(var(--primary-rgb),.2);border-radius:26px;background:rgba(16,16,21,.95);box-shadow:0 30px 100px rgba(0,0,0,.6)}.auth-brand{text-align:center;margin-bottom:25px}.auth-brand img{width:72px;margin:0 auto 12px}.auth-brand h1{margin:0;font-size:25px}.auth-brand p{margin:8px 0 0;color:var(--muted);font-size:11px}.auth-tabs{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-bottom:20px;padding:5px;border:1px solid var(--line);border-radius:16px;background:#0c0c11}.auth-tab{height:43px;border:0;border-radius:12px;background:none;color:var(--muted);font-weight:900;cursor:pointer}.auth-tab.active{background:linear-gradient(135deg,var(--primary),var(--secondary));color:#fff;box-shadow:0 8px 22px rgba(var(--primary-rgb),.16)}.auth-form{display:none}.auth-form.active{display:block}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:11px}.full{grid-column:1/-1}.password-wrap{position:relative}.password-wrap button{position:absolute;left:8px;top:7px;width:37px;height:37px;border:0;background:none;color:var(--muted);cursor:pointer}.auth-links{display:flex;align-items:center;justify-content:space-between;margin:12px 2px 17px;font-size:10px}.link-btn{border:0;background:none;color:#ff7781;cursor:pointer}.auth-error{display:none;margin-bottom:13px;padding:11px 12px;border:1px solid rgba(255,104,129,.22);border-radius:13px;background:rgba(255,104,129,.06);color:#ffb0be;font-size:10px;line-height:1.7}.auth-error.show{display:block}.auth-footer{margin-top:17px;color:var(--muted);font-size:9px;line-height:1.7;text-align:center}
    .toast{position:fixed;z-index:300;right:50%;bottom:calc(var(--bottom) + var(--safe-bottom) + 18px);max-width:min(430px,calc(100% - 24px));padding:12px 16px;border:1px solid rgba(var(--primary-rgb),.2);border-radius:14px;background:#1d1719;color:#fff5f6;box-shadow:0 18px 45px rgba(0,0,0,.42);font-size:11px;line-height:1.6;opacity:0;pointer-events:none;transform:translate(50%,15px);transition:.22s}.toast.show{opacity:1;transform:translate(50%,0)}.toast.error{border-color:rgba(255,104,129,.28)}
...
<body class="locked">
  ...
  <section class="auth-screen" id="authScreen">
    <div class="auth-card">
      <div class="auth-brand"><img src="/app-icon.svg" alt="Uchiha"><h1>Uchiha Store</h1><p>ادخل إلى عالم Uchiha الرقمي بحساب واحد للموقع والبوت</p></div>
      ...
      <div class="auth-footer">هوية Uchiha القرمزية • محفظة وطلبات موحّدة • ربط تيليجرام آمن.</div>
    </div>
  </section>
