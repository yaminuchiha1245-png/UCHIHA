...
async function request(fetchImpl, baseUrl, path, { timeoutMs, accept = "*/*" } = {}) {
  ...
  try {
    const response = await fetchImpl(`${baseUrl}${path}`, {
      redirect: "follow",
      headers: { accept, "user-agent": "UCHIHA-Staging-Smoke/1.0" },
...
export async function runSmoke({
  ...
  const healthResponse = await request(fetchImpl, normalizedBaseUrl, "/health", {
    timeoutMs,
    accept: "application/json"
  });
  if (healthResponse.response.status !== 200) {
    throw new Error(`/health returned HTTP ${healthResponse.response.status}`);
  }
  const health = safeJson(healthResponse.text, "/health");
  assertNoSensitiveKeys(health, "/health");
  if (health.status !== "ok" || health.service !== "uchiha-builder") {
    throw new Error("/health returned an unexpected payload");
  }
  pass("health", { database: health.database });

  const readyResponse = await request(fetchImpl, normalizedBaseUrl, "/ready", {
    timeoutMs,
    accept: "application/json"
  });
  ...
  const configResponse = await request(fetchImpl, normalizedBaseUrl, "/api/public/config", {
    timeoutMs,
    accept: "application/json"
  });
  if (configResponse.response.status !== 200) {
    throw new Error(`/api/public/config returned HTTP ${configResponse.response.status}`);
  }
...
async function main() {
  try {
    const result = await runSmoke({
      ...
      allowDegraded: ["1", "true", "yes", "on"].includes(
        String(process.env.SMOKE_ALLOW_DEGRADED || "").toLowerCase()
      ),
