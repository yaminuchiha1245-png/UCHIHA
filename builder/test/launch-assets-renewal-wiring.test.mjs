...
test("production public aliases are registered as real Fastify routes instead of relying on 404 replacement", () => {
  const routes = [];
  const app = { get(path) { routes.push(path); }, addHook() {} };
  installLaunchAssetInjection(app);
  for (const path of ["/register","/register.html","/index.html","/login.html","/services.html","/api-services","/about","/refund-policy","/privacy.html","/terms.html"]) {
...
test("storefront response upgrades stale runtime asset versions before it reaches the browser", async () => {
  ...
  installLaunchAssetInjection(app);
  const { headers, reply } = replyHarness();
  const oldHtml = `<!doctype html><html><head><link rel="stylesheet" href="/assets/styles.css"><link rel="stylesheet" href="/assets/store-reference.css?v=2026.08.11.2"><script src="/assets/theme.js?v=2026.08.11.2"></script></head><body><script src="/assets/i18n.js" defer></script><script src="/assets/payments-links.js" defer></script><script src="/assets/app.js?v=2026.08.11.2" defer></script></body></html>`;
