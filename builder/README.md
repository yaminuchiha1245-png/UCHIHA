...
- Slider متعدد البنرات يدعم صورة أو GIF أو فيديو مع ترتيب وروابط اختيارية.
...
- بوابة منصة عامة محايدة للخدمات والتواصل وطرق الدفع ونماذج الأعمال وUCHIHA API.
- ترجمة عربية/إنجليزية مع RTL/LTR، ووضع فاتح/داكن محفوظ، وأزرار تبديل ظاهرة.
- لوحة إدارة منصة مستقلة لإدارة الخدمات والطلبات والتواصل والدفع وQR والبنرات والمزودين والسجلات.
...
استخدم قيمًا تجريبية مؤقتة عبر متغيرات البيئة عند اختبار البوتات محليًا. لا تستخدم
توكنات حقيقية في الوضع التجريبي، ولا تحفظ أي سر في المستودع.
...
- يبني مخططًا معزولًا داخل الذاكرة ويحمّل متجر `Nova Digital` التجريبي المحايد.
...
لا توجد كلمات مرور ثابتة داخل الكود. إذا احتجت تسجيل الدخول في بيئة المعاينة، اضبط
`PLATFORM_ADMIN_EMAIL` و`PLATFORM_ADMIN_PASSWORD`، ولحساب العميل التجريبي اضبط
`PREVIEW_CUSTOMER_EMAIL` و`PREVIEW_CUSTOMER_PASSWORD` كمتغيرات بيئة خاصة.
...
في `test_mode` ينشئ الـ Adapter رقم طلب تجريبيًا ولا ينفذ Network Request. وفي `live`
يُحدد نوع الـAdapter وعنوان المزود من لوحة إدارة المنصة، أو من متغيرات البيئة لتهيئة
الخانة الأولى في بيئة فارغة. لا يوجد اسم مزود خارجي مثبت داخل الشيفرة؛
`http-json-v1` هو العقد العام الحالي، ويمكن إضافة Adapters أخرى خلف واجهة
`ProviderAdapter` من دون كشف المزود للعميل.

متغيرات الربط العامة:

```env
UCHIHA_API_1_ADAPTER=http-json-v1
UCHIHA_API_1_BASE_URL=https://provider.example
UCHIHA_API_1_TOKEN=<secret>
```

لا تُحفظ هذه القيم داخل المستودع، وتُخزن بيانات الاعتماد المشفرة في
`api_provider_credentials` مع دعم الرجوع المؤقت للحقل القديم أثناء الترحيل. لكل مزود
سر Webhook مستقل ومشفّر يمكن تدويره من لوحة المنصة؛ يقبل المسار
`/webhooks/providers/:providerId` معرف حدث لمنع التكرار، ولا يخزن حمولة المزود الخام.

## الاستضافة والدومينات المستقبلية

تتضمن Migrations عقود `infrastructure_integrations` و`infrastructure_orders` للاستضافة
والدومينات، مع القدرات والحالة ومفتاح منع التكرار وعزل المتجر. تبدأ الوصلات الافتراضية
بحالة `disabled/not_configured` ولا ترسل طلبات خارجية. عند شراء خدمة API لاحقًا، يضاف
Adapter للمزود وتوضع بيانات اتصاله المشفرة من بيئة التشغيل أو لوحة المنصة، من دون تغيير
واجهات العميل أو كشف اسم المزود الحقيقي.
