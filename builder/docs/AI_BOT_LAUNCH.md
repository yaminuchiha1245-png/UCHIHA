...
الجدول الأساسي يمنع أصلًا مشاركة `telegram_bot_id` نفسه بين نسختين، وMigration 032 تضيف فهرس إطلاق صريحًا ومسمّى تتحقق منه بوابة الجاهزية قبل السماح بالبيع.
...
السيرفر يمنع تحويل المنتج إلى `active` إذا بيئة الإنتاج أو Migration 032 غير جاهزة، كما يمنع الشراء نفسه قبل الخصم إذا أصبحت البيئة غير جاهزة لاحقًا.
...
1. افتح منتج بوت AI.
2. اقرأ بوضوح أن تكلفة OpenAI API منفصلة عن سعر البوت وفعّل مربع الموافقة.
3. نفّذ شراء واحد.
4. تأكد أن الرصيد خُصم مرة واحدة فقط.
5. أعد إرسال نفس محاولة الشراء في حالة انقطاع الشبكة وتأكد أن Idempotency تعيد نفس الطلب بدون خصم ثانٍ.
6. أنشئ Telegram Bot تجريبيًا عبر BotFather.
7. أدخل Bot Token داخل الموقع.
8. أدخل Telegram ID الصحيح للمالك.
9. تأكد أن الموقع يعرض Token مقنّعًا فقط بعد الحفظ.
10. افتح البوت واضغط Start.
11. اكتب `/admin` من حساب المالك.
12. جرّب `/admin` من حساب آخر وتأكد أنه مرفوض.
