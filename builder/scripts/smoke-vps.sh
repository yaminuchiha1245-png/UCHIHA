...
READY_BODY="$(mktemp)"
STORE_BODY="$(mktemp)"
trap 'rm -f "$HOME_HEADERS" "$HOME_BODY" "$SERVICES_BODY" "$PAYMENT_METHODS_BODY" "$ORDERS_BODY" "$PORTAL_BODY" "$READY_BODY" "$STORE_BODY"' EXIT
...
printf 'PASS UCHIHA Builder production homepage replaces v41\n'
...
printf 'PASS services, payment methods and orders use one UCHIHA Builder shell\n'
