...
HOME_HEADERS="$(mktemp)"
HOME_BODY="$(mktemp)"
trap 'rm -f "$HOME_HEADERS" "$HOME_BODY" /tmp/uchiha-smoke-body /tmp/uchiha-demo-host /tmp/uchiha-platform-client /tmp/uchiha-recovery-client' EXIT
...
printf 'PASS resilient public shell release %s\n' "$PUBLIC_RELEASE"
