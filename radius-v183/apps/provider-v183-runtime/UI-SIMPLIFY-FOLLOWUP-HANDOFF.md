...
## Integrated release-candidate preservation

The integration branch retains the existing custom subscriber form (independent USD/SYP/TRY exact prices, upload/download Mbps and daily quota). Only the authorized Site Agent navigation guard is applied from this UI follow-up in `runtime.js`; the API and all subscriber creation wiring stay as reviewed in integration PR #83 and subscriber PR #85. The new UI CSS combines the validated 2-column KPI and all three network-health indicators with the candidate's custom subscriber field styles. This branch is isolated and not a VPS deployment.
