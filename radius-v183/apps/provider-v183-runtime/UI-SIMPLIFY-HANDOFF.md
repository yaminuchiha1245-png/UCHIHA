...
- The nine deep-link tests previously passed on this same UI branch before this CSS-only correction. The earlier combined `ui-*.test.mjs` suite passed **20/20** before the new health regression test. Re-run the **21-test combined suite**, standalone web build, Android asset preparation, and complete project checks before release.
