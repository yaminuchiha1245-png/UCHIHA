...
const mock={
 ...
 providers:[
  {id:"demo",name:"Demo Provider",type:"demo",active:true,priority:1,timeoutMs:12000,secretEnv:null,secretConfigured:false,baseUrl:null,orderPath:null},
  {id:"manual",name:"Manual Fulfillment",type:"manual",active:true,priority:99,timeoutMs:12000,secretEnv:null,secretConfigured:false,baseUrl:null,orderPath:null,statusPath:null},
  {id:"inventory",name:"Digital Inventory",type:"inventory",active:true,priority:0,timeoutMs:1000,secretEnv:null,secretConfigured:false,baseUrl:null,orderPath:null,statusPath:null}
 ],
 providerLogs:[
...
function renderProviders(){
 ...
 $("#providersTable").innerHTML=`<table><thead><tr><th>المزود</th><th>النوع</th><th>الأولوية</th><th>Timeout</th><th>توكن API</th><th>الحالة</th><th>اختبار</th><th>إجراء</th></tr></thead><tbody>${ps.map(p=>`<tr><td><span class="provider-dot ${p.active?"":"off"}"></span><b>${esc(p.name)}</b><br><small>${esc(p.id)}</small></td><td>${esc(p.type)}</td><td>${Number(p.priority||0)}</td><td>${Number(p.timeoutMs||0)}ms</td><td>${p.secretConfigured?'<span class="pill ok">🔒 محفوظ</span>':(p.authMode==="none"?'<span class="pill ok">غير مطلوب</span>':'<span class="pill warn">غير مضبوط</span>')}</td><td>${p.active?'<span class="pill ok">فعال</span>':'<span class="pill bad">متوقف</span>'}</td><td><button data-action="test-provider" data-id="${attr(p.id)}">اختبار</button></td><td><button data-action="edit-provider" data-id="${attr(p.id)}">تعديل</button></td></tr>`).join("")||rowEmpty(8)}</tbody></table>`;
}
...
function categoryDepth(id){let depth=0,current=(data.categories||[]).find(c=>c.id===id),seen=new Set();while(current?.parentId&&!seen.has(current.id)&&depth<20){seen.add(current.id);depth++;current=(data.categories||[]).find(c=>c.id===current.parentId)}return depth}
...
function openCategoryCreate(level=0){
