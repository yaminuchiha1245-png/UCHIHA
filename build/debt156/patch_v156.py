#!/usr/bin/env python3
"""Overlay the real v1.5.6 digital store onto the approved v1.4.5 debt app."""
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: patch_v156.py <debt-app>")
    ...
    hero_source = repo_root / "static" / "uchiha-hero-market.webp"
    hero_target = app / "app" / "src" / "main" / "assets" / "digital-hero-v156.webp"
    if hero_source.is_file():
        shutil.copy2(hero_source, hero_target)

    sprite_b64 = Path(__file__).resolve().parent / "assets" / "smm-icons-v156.webp.b64"
    sprite_target = app / "app" / "src" / "main" / "assets" / "smm-icons-v156.webp"
    if not sprite_b64.is_file():
        raise SystemExit("missing local SMM icon sprite")
    sprite_target.write_bytes(base64.b64decode(sprite_b64.read_text(encoding="ascii").strip()))

    index_path = app / "app" / "src" / "main" / "assets" / "index.html"
    index = index_path.read_text(encoding="utf-8")
    index = replace_once(index, '<link rel="stylesheet" href="app-v145.css">', '<link rel="stylesheet" href="app-v145.css">\n  <link rel="stylesheet" href="app-v156.css">', "v156 css")
    index = replace_once(index, '<script src="app-v145.js"></script>', '<script src="app-v145.js"></script>\n  <script src="app-v156.js"></script>', "v156 js")
...
'''
    ...
    build = build.replace("versionCode 1040500", "versionCode 1050600", 1)
    build = build.replace("versionName '1.4.5'", "versionName '1.5.6'", 1)
