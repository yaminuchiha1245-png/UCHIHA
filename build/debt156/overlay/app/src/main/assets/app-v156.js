/* UCHIHA Debt Store v1.5.6 — approved SMM layout. */
(function(){'use strict';
const VERSION='1.5.6';
...
const SMM_APPS=[
  ...
  {key:'whatsapp',name:'WhatsApp',sx:1,sy:3,rx:/whatsapp|واتساب|واتس اب|واتسآب/i},
  {key:'threads',name:'Threads',sx:2,sy:3,rx:/threads|ثريدز|ثريد/i},
  {key:'pinterest',name:'Pinterest',sx:3,sy:3,rx:/pinterest|بنترست|بينترست/i}
];
...
function smmLogo(meta,cls=''){
  const m=meta||{sx:0,sy:0};
  return '<span class="smm-logo '+cls+'" style="--sx:'+Number(m.sx||0)+';--sy:'+Number(m.sy||0)+'"></span>';
}
...
window.smmUpdatePrice=smmUpdatePrice;window.smmToggleDrop=smmToggleDrop;window.smmDropFilter=smmDropFilter;
function smmPlatformChips(){
  const rows=smm.platforms||[],limit=13,shown=smm.expanded?rows:rows.slice(0,limit);
  if(!rows.length)return '<div class="smm-app-empty">لم يتم العثور على تطبيقات رشق ضمن أقسام المزود.</div>';
  let html='<section class="smm-app-panel"><div class="smm-platforms">';
  html+='<button class="smm-all '+(smm.platform?.key==='all'?'active':'')+'" onclick="DigitalStore.smmPlatform(\'all\')">الكل</button>';
  html+=shown.map(p=>{const m=smmPlatformMeta(p),active=smm.platform&&smm.platform.key===p.key;return '<button class="smm-app '+(active?'active':'')+'" onclick="DigitalStore.smmPlatform(\''+safe(p.key)+'\')" title="'+safe(p.name)+'">'+smmLogo(m)+'</button>'}).join('');
  html+='</div>';
...
function smmOrderForm(){
  if(!smm.platform){
    return '<section class="smm-order-card smm-await-app"><div class="smm-app-prompt"><b>اختر التطبيق أولًا</b><span>اختر التطبيق من الأعلى، وبعدها تظهر أنواع الخدمات كما هي لدى المزود.</span></div></section>';
  }
  ...
  if(p){
    const fieldsHtml=fields.map((f,i)=>{
      ...
      return '<div class="smm-field"><label>'+safe(label)+(f.required?' *':'')+'</label>'+
        (f.options.length?'<select class="smm-select smm-order-field" id="smmF'+i+'" data-key="'+safe(f.key)+'">'+f.options.map(o=>'<option value="'+safe(o.value)+'">'+safe(o.label)+'</option>').join('')+'</select>':'<div class="smm-input-wrap">'+(isLink?'<i>🔗</i>':'')+'<input class="smm-input smm-order-field" id="smmF'+i+'" data-key="'+safe(f.key)+'" '+(f.required?'required':'')+' placeholder="'+safe(isLink?'أدخل رابط الحساب أو المنشور':f.label)+'"></div>')+'</div>';
    }).join('');
    let qtyHtml='';
    if(q.variable){
      qtyHtml='<div class="smm-field"><label>الكمية</label>'+
        (q.options.length?'<select class="smm-select" id="smmQty" onchange="smmUpdatePrice()">'+q.options.map(v=>'<option value="'+v+'">'+v+'</option>').join('')+'</select>':'<input class="smm-input smm-qty" id="smmQty" type="number" inputmode="numeric" min="'+q.min+'" max="'+q.max+'" step="'+q.step+'" value="'+q.min+'" oninput="smmUpdatePrice()">')+
    ...
    detail=fieldsHtml+qtyHtml+
      ...
      '<button class="smm-confirm" onclick="DigitalStore.smmBuy()">شراء</button>';
  }
...
function smmScreen(){
  const root='<div class="digital-store smm-store"><div class="digital-wrap">'+header()+statusStrip()+
    '<div class="smm-page-head"><div><h2>قسم الرشق 🚀</h2><small>خدمات السوشيال ميديا بأفضل جودة وأسعار</small></div></div>'+
    smmPlatformChips()+
...
function hero(){
  return '<section class="digital-hero"><img class="digital-hero-real" src="digital-hero-v156.webp" alt=""><div class="digital-hero-shade"></div><div class="digital-hero-copy"><b><em>DIGITAL</em><br>WORLD<br>WITH <em>UCHIHA</em></b><span>كل ما تحتاجه في مكان واحد</span></div></section>';
}
function topupModal(){
  if(!topupOpen)return'';
...
window.DigitalStore={
  ...
  async openSmm(row){
    ...
    smm.rootCategories=r.data.categories||[];smm.rootProducts=r.data.products||[];smm.platforms=smmBuildApps(smm.rootCategories,smm.rootProducts);smm.sections=[];smm.sectionOptions=[];smm.products=[];smmScreen();
  },
  ...
  async smmPlatform(key){
    ...
    if(String(key)==='all'){
      smm.platform={key:'all',name:'الكل',sx:0,sy:0};
