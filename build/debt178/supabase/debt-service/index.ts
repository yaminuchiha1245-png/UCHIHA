// UCHIHA Debt Service v1.5.28 — idempotent financial requests + schema-driven orders.
...
async function serviceRpc(name:string,args:Record<string,unknown>){
  ...
  if(!allowed.has(name))throw new Error('DATABASE');
  const res=await fetch(Deno.env.get('SUPABASE_URL')+'/rest/v1/rpc/'+name,{
    method:'POST',headers:dbHeaders(),body:JSON.stringify(args),signal:AbortSignal.timeout(25000)
  });
  if(!res.ok)throw new Error('DATABASE');
  return await res.json();
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==='digital_topup_create'){
      ...
      const supplied=clampText(input.request_id,100);
      if(supplied&&!/^[A-Za-z0-9:_-]{12,100}$/.test(supplied))return response({ok:false,error:'INVALID_REQUEST'},400);
      const requestId=supplied||('legacy:'+crypto.randomUUID());
      const out=await serviceRpc('debt_digital_topup_create_idempotent',{
        p_actor:actor,p_request_id:requestId,p_amount:amount,p_proof_data:proof
      });
      return response(out,out.ok?200:400);
    }
    ...
    if(action==='digital_purchase'){
      ...
      if(supplied&&!/^[A-Za-z0-9:_-]{12,100}$/.test(supplied))return response({ok:false,error:'INVALID_REQUEST'},400);
      const requestId=supplied||('legacy:'+crypto.randomUUID());
      const begin=await serviceRpc('debt_digital_purchase_begin_idempotent',{
        p_actor:actor,p_request_id:requestId,p_product_id:productId,
        p_product_name:String(product.name||product.title||('Product '+productId)).slice(0,250),
        p_amount:amount,p_quantity:qty,p_fields:fields
      });
    ...
    if(action==='owner_digital_wallet_adjust'){
      const target=clampText(input.license_id,60),amount=number(input.amount,-1000000,1000000),reason=clampText(input.reason,300);
      const supplied=clampText(input.request_id,100);
      if(supplied&&!/^[A-Za-z0-9:_-]{12,100}$/.test(supplied))return response({ok:false,error:'INVALID_REQUEST'},400);
      const requestId=supplied||('legacy:'+crypto.randomUUID());
      const out=await serviceRpc('debt_digital_wallet_adjust_idempotent',{
        p_actor:actor,p_target:target,p_request_id:requestId,p_amount:amount,p_reason:reason
      });
      return response(out,out.ok?200:400);
    }
