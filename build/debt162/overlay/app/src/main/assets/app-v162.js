/* UCHIHA Debt Store v1.5.12 — flat app icons, stable keyboard, native bottom nav. */
(function(){'use strict';
...
const SMM_APPS=[
  {key:'instagram',name:'Instagram',sx:1,sy:1,rx:/instagram|insta|انستغرام|انستقرام|انستا/i},
  {key:'tiktok',name:'TikTok',sx:0,sy:1,rx:/tiktok|tik tok|تيك\s*توك|تيكتوك/i},
  {key:'youtube',name:'YouTube',sx:4,sy:0,rx:/youtube|youtu\.be|يوتيوب/i},
  ...
  {key:'spotify',name:'Spotify',sx:4,sy:2,rx:/spotify|سبوتيفاي|سبوتفاي/i},
  {key:'snapchat',name:'Snapchat',sx:3,sy:2,rx:/snapchat|snap chat|سناب\s*شات|سناب/i},
  {key:'linkedin',name:'LinkedIn',sx:2,sy:2,rx:/linkedin|لينكد\s*ان|لينكدإن/i},
  {key:'twitch',name:'Twitch',sx:1,sy:2,rx:/twitch|تويتش/i},
  {key:'kick',name:'Kick',sx:0,sy:2,rx:/\bkick\b|كيك/i},
  {key:'whatsapp',name:'WhatsApp',sx:2,sy:1,rx:/whatsapp|واتساب|واتس اب|واتسآب/i},
  {key:'quora',name:'Quora',sx:3,sy:1,rx:/quora|كورا/i},
  {key:'soundcloud',name:'SoundCloud',sx:4,sy:1,rx:/soundcloud|ساوند\s*كلاود|ساوندكلاود/i},
  {key:'tumblr',name:'Tumblr',sx:1,sy:3,rx:/tumblr|تمبلر|تومبلر/i},
  {key:'threads',name:'Threads',sx:2,sy:3,rx:/threads|ثريدز|ثريد/i},
  {key:'cloudsync',name:'Cloud Sync',sx:3,sy:3,rx:/cloud\s*sync|sync\s*cloud|مزامنة\s*سحاب|سحابي/i}
];
...
function smmLogo(meta,cls=''){
  ...
  return '<span class="smm-logo '+cls+'" data-app="'+safe(m.key)+'" style="--sx:'+Number(m.sx||0)+';--sy:'+Number(m.sy||0)+'"></span>';
}
...
function smmDropFilter(v){
  smm.dropSearch=String(v||'');
  const q=smm.dropSearch.trim().toLowerCase();
...
window.DigitalStore={
  ...
  smmSearch(v){
    smm.search=String(v||'');
    const q=smm.search.trim().toLowerCase();
    document.querySelectorAll('.smm-app').forEach(btn=>{
      const label=String(btn.getAttribute('title')||'').toLowerCase();
