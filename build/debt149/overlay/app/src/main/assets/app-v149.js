/* UCHIHA Debt Store v1.4.9 — digital store runtime fix. */
(function(){'use strict';
const VERSION='1.4.9';
...
async function loadCatalog(categoryId=0){
  ...
  catalog=arrays(r.data);lastCatalogError='';storeScreen();
}
...
window.DigitalStore={
  async open(){
    ...
    mode='store';path=[];query='';catalog={categories:[],products:[]};lastCatalogError='';drawerOpen=false;loading=true;
    try{storeScreen();}catch(_e){loading=false;notify('تعذر فتح واجهة المنتجات الرقمية',true);return;}
  ...
  root(){if(mode!=='store'){mode='store'}path=[];query='';lastCatalogError='';loadCatalog(0)},
  async category(id){const row=(catalog.categories||[]).find(x=>Number(x.id)===Number(id));if(row)path.push(row);query='';lastCatalogError='';await loadCatalog(id)},
