/* UCHIHA Debt Store v1.5.3 — specialized SMM ordering experience. */
(function(){'use strict';
const VERSION='1.5.3';
...
let smm={root:null,returnPath:[],platforms:[],platform:null,sectionTrail:[],sections:[],products:[],product:null,search:'',expanded:false,error:''};
...
function smmPlatformMeta(item){
  const n=nameOf(item).toLowerCase();
  if(/instagram|انستا/.test(n))return{glyph:'IG',cls:'ig'};
  if(/tiktok|تيك/.test(n))return{glyph:'TT',cls:'tt'};
  if(/youtube|يوتيوب/.test(n))return{glyph:'YT',cls:'yt'};
  if(/facebook|فيس/.test(n))return{glyph:'f',cls:'fb'};
  if(/telegram|تلغ|تليغ/.test(n))return{glyph:'TG',cls:'tg'};
  if(/twitter|\bx\b|تويتر/.test(n))return{glyph:'X',cls:'x'};
  if(/snap|سناب/.test(n))return{glyph:'SC',cls:'sc'};
  if(/discord|ديسكورد/.test(n))return{glyph:'DC',cls:'dc'};
  if(/spotify|سبوت/.test(n))return{glyph:'SP',cls:'sp'};
  if(/linkedin|لينكد/.test(n))return{glyph:'in',cls:'li'};
  if(/twitch|تويتش/.test(n))return{glyph:'TW',cls:'tw'};
  if(/reddit|ريديت/.test(n))return{glyph:'R',cls:'rd'};
  const clean=nameOf(item).replace(/[^\p{L}\p{N}]/gu,'').slice(0,2).toUpperCase();
  return{glyph:clean||'S',cls:'default'};
}
...
function smmPlatformChips(){
  const rows=smm.platforms||[],limit=7,shown=smm.expanded?rows:rows.slice(0,limit);
  let html='<div class="smm-platforms"><button class="smm-chip all '+(!smm.platform?'active':'')+'" onclick="DigitalStore.smmPlatform(0)"><span>الكل</span></button>';
  html+=shown.map(p=>{const m=smmPlatformMeta(p),active=smm.platform&&Number(smm.platform.id)===Number(p.id);return '<button class="smm-chip '+(active?'active ':'')+safe(m.cls)+'" onclick="DigitalStore.smmPlatform('+Number(p.id||0)+')"><i>'+safe(m.glyph)+'</i><span>'+safe(nameOf(p))+'</span></button>'}).join('');
  html+='</div>';
...
function smmOrderForm(){
  const sections=smm.sections||[],products=(smm.products||[]).filter(p=>!smm.search||nameOf(p).toLowerCase().includes(smm.search.toLowerCase()));
  ...
  const sectionSelect=sections.length?
    '<div class="smm-field"><label>القسم</label><select class="smm-select" onchange="DigitalStore.smmSection(this.value)"><option value="">اختر القسم</option>'+sections.map(x=>'<option value="'+Number(x.id||0)+'">'+safe(nameOf(x))+'</option>').join('')+'</select>'+sectionTrail+'</div>':
    '<div class="smm-field"><label>القسم</label><div class="smm-static">'+safe(smm.platform?nameOf(smm.platform):nameOf(smm.root||{name:'الرشق'}))+'</div>'+sectionTrail+'</div>';
  const productSelect='<div class="smm-field"><label>الخدمة</label><select class="smm-select" onchange="DigitalStore.smmService(this.value)" '+(!products.length?'disabled':'')+'><option value="">'+(products.length?'اختر الخدمة':'اختر القسم أولًا')+'</option>'+products.map(x=>'<option value="'+Number(x.id||0)+'" '+(p&&Number(p.id)===Number(x.id)?'selected':'')+'>'+safe(nameOf(x))+'</option>').join('')+'</select></div>';
  let detail='';
  if(p){
...
function smmScreen(){
  const root='<div class="digital-store smm-store"><div class="digital-wrap">'+header()+statusStrip()+
    '<div class="smm-page-head"><button class="digital-back" onclick="DigitalStore.smmExit()">‹</button><div><small>الخدمات الاجتماعية</small><h2>قسم الرشق</h2></div></div>'+
    smmPlatformChips()+
...
window.DigitalStore={
  ...
  async openSmm(row){
    smm={root:row,returnPath:[...path],platforms:[],platform:null,sectionTrail:[],sections:[],products:[],product:null,search:'',expanded:false,error:''};
    ...
    smm.platforms=r.data.categories||[];smm.sections=[];smm.products=r.data.products||[];smmScreen();
  },
  smmMore(){smm.expanded=!smm.expanded;smmScreen()},
  async smmPlatform(id){
    if(loading)return;
    smm.product=null;smm.sectionTrail=[];smm.sections=[];smm.products=[];smm.search='';smm.error='';
    if(!Number(id)){
      smm.platform=null;loading=true;smmScreen();
      const r=await smmFetchCatalog(smm.root?.id);loading=false;
      if(!r.ok){smm.error=errorText(r.error);smmScreen();return;}
      smm.platforms=r.data.categories||smm.platforms;smm.sections=[];smm.products=r.data.products||[];smmScreen();return;
    }
    const row=(smm.platforms||[]).find(x=>Number(x.id)===Number(id));if(!row)return;
    smm.platform=row;loading=true;smmScreen();
    const r=await smmFetchCatalog(row.id);loading=false;
    if(!r.ok){smm.error=errorText(r.error);smmScreen();return;}
    smm.sections=r.data.categories||[];smm.products=r.data.products||[];smmScreen();
  },
  async smmSection(id){
  ...
  smmNew(){smm.product=null;smm.search='';smm.sectionTrail=[];if(smm.platform){this.smmPlatform(smm.platform.id)}else{smm.sections=[];smm.products=[];this.smmPlatform(0)}},
  smmRetry(){if(smm.platform)this.smmPlatform(smm.platform.id);else this.smmPlatform(0)},
  smmExit(){
