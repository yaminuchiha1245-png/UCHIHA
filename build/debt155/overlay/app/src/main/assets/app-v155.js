/* UCHIHA Debt Store v1.5.5 — real app logos + no service description. */
(function(){'use strict';
const VERSION='1.5.5';
...
let smm={root:null,returnPath:[],rootCategories:[],rootProducts:[],platforms:[],platform:null,sectionTrail:[],sections:[],products:[],product:null,search:'',expanded:false,error:''};
...
const SMM_APPS=[
  {key:'instagram',name:'Instagram',icon:'https://cdn.simpleicons.org/instagram',bg:'linear-gradient(135deg,#f9ce34,#ee2a7b 48%,#6228d7)',rx:/instagram|insta|انستغرام|انستقرام|انستا/i},
  {key:'tiktok',name:'TikTok',icon:'https://cdn.simpleicons.org/tiktok/ffffff',bg:'#070a10',rx:/tiktok|tik tok|تيك\s*توك|تيكتوك/i},
  {key:'youtube',name:'YouTube',icon:'https://cdn.simpleicons.org/youtube/ffffff',bg:'#ff0033',rx:/youtube|youtu\.be|يوتيوب/i},
  {key:'facebook',name:'Facebook',icon:'https://cdn.simpleicons.org/facebook/ffffff',bg:'#1877f2',rx:/facebook|fb\b|فيسبوك|فيس بوك/i},
  {key:'telegram',name:'Telegram',icon:'https://cdn.simpleicons.org/telegram/ffffff',bg:'#229ed9',rx:/telegram|تلغرام|تليغرام|تيليجرام|تليجرام/i},
  {key:'whatsapp',name:'WhatsApp',icon:'https://cdn.simpleicons.org/whatsapp/ffffff',bg:'#25d366',rx:/whatsapp|واتساب|واتس اب|واتسآب/i},
  {key:'snapchat',name:'Snapchat',icon:'https://cdn.simpleicons.org/snapchat/000000',bg:'#fffc00',rx:/snapchat|snap chat|سناب\s*شات|سناب/i},
  {key:'twitter',name:'X',icon:'https://cdn.simpleicons.org/x/ffffff',bg:'#05070a',rx:/twitter|تويتر|\bX\b/i},
  {key:'threads',name:'Threads',icon:'https://cdn.simpleicons.org/threads/ffffff',bg:'#111111',rx:/threads|ثريدز|ثريد/i},
  {key:'discord',name:'Discord',icon:'https://cdn.simpleicons.org/discord/ffffff',bg:'#5865f2',rx:/discord|ديسكورد/i},
  {key:'spotify',name:'Spotify',icon:'https://cdn.simpleicons.org/spotify/000000',bg:'#1ed760',rx:/spotify|سبوتيفاي|سبوتفاي/i},
  {key:'linkedin',name:'LinkedIn',icon:'https://cdn.simpleicons.org/linkedin/ffffff',bg:'#0a66c2',rx:/linkedin|لينكد\s*ان|لينكدإن/i},
  {key:'twitch',name:'Twitch',icon:'https://cdn.simpleicons.org/twitch/ffffff',bg:'#9146ff',rx:/twitch|تويتش/i},
  {key:'reddit',name:'Reddit',icon:'https://cdn.simpleicons.org/reddit/ffffff',bg:'#ff4500',rx:/reddit|ريديت/i},
  {key:'pinterest',name:'Pinterest',icon:'https://cdn.simpleicons.org/pinterest/ffffff',bg:'#e60023',rx:/pinterest|بنترست|بينترست/i},
  {key:'soundcloud',name:'SoundCloud',icon:'https://cdn.simpleicons.org/soundcloud/ffffff',bg:'#ff5500',rx:/soundcloud|ساوند\s*كلاود|ساوندكلاود/i},
  {key:'kick',name:'Kick',icon:'https://cdn.simpleicons.org/kick/0b0f0d',bg:'#53fc18',rx:/\bkick\b|كيك/i}
];
...
function smmPlatformMeta(item){
  ...
  return SMM_APPS.find(a=>a.key===key)||{icon:'',bg:'#16314a',name:String(item?.name||'')};
}
...
function smmPriceNow(){
  ...
  const n=Number(el('smmQty')?.value||q.min||1);
...
function smmPlatformChips(){
  const rows=smm.platforms||[],limit=8,shown=smm.expanded?rows:rows.slice(0,limit);
  if(!rows.length)return '<div class="smm-app-empty">لم يتم العثور على تطبيقات رشق ضمن أقسام المزود.</div>';
  let html='<div class="smm-platforms">';
  html+=shown.map(p=>{const m=smmPlatformMeta(p),active=smm.platform&&smm.platform.key===p.key;const icon=m.icon?'<img src="'+safe(m.icon)+'" alt="'+safe(p.name)+'" referrerpolicy="no-referrer">':'<b>'+safe(p.name.slice(0,2))+'</b>';return '<button class="smm-chip '+(active?'active ':'')+'" onclick="DigitalStore.smmPlatform(\''+safe(p.key)+'\')" title="'+safe(p.name)+'"><i style="background:'+safe(m.bg||'#17314a')+'">'+icon+'</i><span>'+safe(p.name)+'</span></button>'}).join('');
  html+='</div>';
  if(rows.length>limit&&!smm.expanded)html+='<button class="smm-more" onclick="DigitalStore.smmMore()">عرض المزيد +'+(rows.length-limit)+'</button>';
  else if(rows.length>limit)html+='<button class="smm-more" onclick="DigitalStore.smmMore()">عرض أقل</button>';
  return html;
}
function smmOrderForm(){
  if(!smm.platform){
    return '<section class="smm-order-card smm-await-app"><div class="smm-app-prompt"><b>اختر التطبيق أولًا</b><span>اختر Instagram أو WhatsApp أو أي تطبيق آخر من الأعلى، وبعدها تظهر أنواع الخدمات كما هي مرتبة لدى جاسر كارد.</span></div></section>';
  }
  const rawSections=smm.sections||[];
  const qText=smm.search.trim().toLowerCase();
  const sections=qText?rawSections.filter(x=>nameOf(x).toLowerCase().includes(qText)):rawSections;
  const rawProducts=smm.products||[];
  const products=qText?rawProducts.filter(p=>nameOf(p).toLowerCase().includes(qText)):rawProducts;
  const p=smm.product,q=p?quantityMeta(p):null,fields=p?smmLinkFields(p):[];
  const sectionTrail=smm.sectionTrail.length?'<small class="smm-trail">'+smm.sectionTrail.map(x=>safe(nameOf(x))).join(' ‹ ')+'</small>':'';
  let sectionSelect='';
  if(sections.length){
    sectionSelect='<div class="smm-field"><label>نوع الخدمة</label><select class="smm-select" onchange="DigitalStore.smmSection(this.value)"><option value="">اختر نوع الخدمة كما هو لدى المزود</option>'+sections.map(x=>'<option value="'+Number(x.id||0)+'">'+safe(nameOf(x))+'</option>').join('')+'</select>'+sectionTrail+'</div>';
  }else{
    const m=smmPlatformMeta(smm.platform);sectionSelect='<div class="smm-field"><label>التطبيق</label><div class="smm-static smm-static-app"><i style="background:'+safe(m.bg||'#17314a')+'"><img src="'+safe(m.icon||'')+'" alt=""></i><span>'+safe(smm.platform.name)+'</span></div>'+sectionTrail+'</div>';
  }
  const productSelect='<div class="smm-field"><label>الخدمة</label><select class="smm-select" onchange="DigitalStore.smmService(this.value)" '+(!products.length?'disabled':'')+'><option value="">'+(products.length?'اختر الخدمة':'اختر نوع الخدمة أولًا')+'</option>'+products.map(x=>'<option value="'+Number(x.id||0)+'" '+(p&&Number(p.id)===Number(x.id)?'selected':'')+'>'+safe(nameOf(x))+'</option>').join('')+'</select></div>';
  let detail='';
  if(p){
    const fieldsHtml=fields.map((f,i)=>'<div class="smm-field"><label>'+safe(f.label)+(f.required?' *':'')+'</label>'+
      (f.options.length?'<select class="smm-select smm-order-field" id="smmF'+i+'" data-key="'+safe(f.key)+'">'+f.options.map(o=>'<option value="'+safe(o.value)+'">'+safe(o.label)+'</option>').join('')+'</select>':'<input class="smm-input smm-order-field" id="smmF'+i+'" data-key="'+safe(f.key)+'" '+(f.required?'required':'')+' placeholder="'+safe(f.label)+'">')+'</div>').join('');
    let qtyHtml='';
    if(q.variable){
      qtyHtml='<div class="smm-field"><label>الكمية</label>'+
        (q.options.length?'<select class="smm-select" id="smmQty" onchange="smmUpdatePrice()">'+q.options.map(v=>'<option value="'+v+'">'+v+'</option>').join('')+'</select>':'<input class="smm-input" id="smmQty" type="number" inputmode="numeric" min="'+q.min+'" max="'+q.max+'" step="'+q.step+'" value="'+q.min+'" oninput="smmUpdatePrice()">')+
        '<small class="smm-limits">الحد الأدنى: '+q.min+' · الحد الأقصى: '+q.max+'</small></div>';
    }
    detail=fieldsHtml+qtyHtml+
      '<div class="smm-price-row"><span>ثمن الطلب</span><strong id="smmPriceValue">'+safe(money(q&&q.variable?priceOf(p)*q.min:priceOf(p)))+'</strong></div>'+
      '<button class="smm-confirm" onclick="DigitalStore.smmBuy()">تأكيد الطلب</button>';
  }
  return '<section class="smm-order-card">'+sectionSelect+productSelect+detail+'</section>';
}
function smmScreen(){
  const root='<div class="digital-store smm-store"><div class="digital-wrap">'+header()+statusStrip()+
    '<div class="smm-page-head"><button class="digital-back" onclick="DigitalStore.smmExit()">‹</button><div><small>اختر التطبيق ثم نوع الخدمة</small><h2>قسم الرشق</h2></div></div>'+
    smmPlatformChips()+
    '<button class="smm-new" onclick="DigitalStore.smmNew()">طلب جديد</button>'+
    '<div class="smm-search"><i>⌕</i><input value="'+safe(smm.search)+'" placeholder="بحث" oninput="DigitalStore.smmSearch(this.value)"></div>'+
...
window.DigitalStore={
  ...
  async openSmm(row){
    smm={root:row,returnPath:[...path],rootCategories:[],rootProducts:[],platforms:[],platform:null,sectionTrail:[],sections:[],products:[],product:null,search:'',expanded:false,error:''};
    ...
    smm.rootCategories=r.data.categories||[];smm.rootProducts=r.data.products||[];smm.platforms=smmBuildApps(smm.rootCategories,smm.rootProducts);smm.sections=[];smm.products=[];smmScreen();
  },
  ...
  async smmPlatform(key){
    ...
    smm.platform=app;smm.product=null;smm.sectionTrail=[];smm.search='';smm.error='';
  ...
  async smmSection(id){
    ...
    const row=(smm.sections||[]).find(x=>Number(x.id)===Number(id));if(!row)return;
    loading=true;smmScreen();
    const r=await smmFetchCatalog(row.id);loading=false;
    if(!r.ok){smm.error=errorText(r.error);smmScreen();return;}
    smm.sectionTrail=[...smm.sectionTrail,row];
    smm.sections=r.data.categories||[];
    smm.products=r.data.products||[];
    smm.product=null;smm.search='';smmScreen();
  },
  async smmService(id){
    if(!id||loading){smm.product=null;smmScreen();return;}
    loading=true;smmScreen();
  ...
  smmNew(){smm.product=null;smm.search='';smm.sectionTrail=[];if(smm.platform)this.smmPlatform(smm.platform.key);else{smm.sections=[];smm.products=[];smmScreen()}},
  ...
  exit(){topupOpen=false;proofData='';proofName='';selectedProduct=null;path=[];query='';smm.product=null;smm.platform=null;mode='';render()}
};
