/* UCHIHA Debt Store v1.5.4 — SMM apps separated from provider services. */
(function(){'use strict';
const VERSION='1.5.4';
...
const SMM_APPS=[
  {key:'instagram',name:'Instagram',glyph:'IG',cls:'ig',rx:/instagram|insta|انستغرام|انستقرام|انستا/i},
  {key:'tiktok',name:'TikTok',glyph:'TT',cls:'tt',rx:/tiktok|tik tok|تيك\s*توك|تيكتوك/i},
  {key:'youtube',name:'YouTube',glyph:'YT',cls:'yt',rx:/youtube|youtu\.be|يوتيوب/i},
  {key:'facebook',name:'Facebook',glyph:'f',cls:'fb',rx:/facebook|fb\b|فيسبوك|فيس بوك/i},
  {key:'telegram',name:'Telegram',glyph:'TG',cls:'tg',rx:/telegram|تلغرام|تليغرام|تيليجرام|تليجرام/i},
  {key:'whatsapp',name:'WhatsApp',glyph:'WA',cls:'wa',rx:/whatsapp|واتساب|واتس اب|واتسآب/i},
  {key:'snapchat',name:'Snapchat',glyph:'SC',cls:'sc',rx:/snapchat|snap chat|سناب\s*شات|سناب/i},
  {key:'twitter',name:'X',glyph:'X',cls:'x',rx:/twitter|تويتر|\bX\b/i},
  {key:'threads',name:'Threads',glyph:'TH',cls:'th',rx:/threads|ثريدز|ثريد/i},
  {key:'discord',name:'Discord',glyph:'DC',cls:'dc',rx:/discord|ديسكورد/i},
  {key:'spotify',name:'Spotify',glyph:'SP',cls:'sp',rx:/spotify|سبوتيفاي|سبوتفاي/i},
  {key:'linkedin',name:'LinkedIn',glyph:'in',cls:'li',rx:/linkedin|لينكد\s*ان|لينكدإن/i},
  {key:'twitch',name:'Twitch',glyph:'TW',cls:'tw',rx:/twitch|تويتش/i},
  {key:'reddit',name:'Reddit',glyph:'R',cls:'rd',rx:/reddit|ريديت/i},
  {key:'pinterest',name:'Pinterest',glyph:'P',cls:'pt',rx:/pinterest|بنترست|بينترست/i},
  {key:'soundcloud',name:'SoundCloud',glyph:'SC',cls:'sound',rx:/soundcloud|ساوند\s*كلاود|ساوندكلاود/i},
  {key:'kick',name:'Kick',glyph:'K',cls:'kick',rx:/\bkick\b|كيك/i}
];
...
function smmPlatformMeta(item){
  ...
  return SMM_APPS.find(a=>a.key===key)||{glyph:'S',cls:'default',name:String(item?.name||'')};
}
...
function smmDescription(p){
  for(const k of ['description','desc','details','instructions','instruction','note','notes','text','product_description']){
    const v=p?.[k];if(typeof v==='string'&&v.trim())return v.trim();
  }
  return'';
}
...
function smmPlatformChips(){
  ...
  html+=shown.map(p=>{const m=smmPlatformMeta(p),active=smm.platform&&smm.platform.key===p.key;return '<button class="smm-chip '+(active?'active ':'')+safe(m.cls)+'" onclick="DigitalStore.smmPlatform(\''+safe(p.key)+'\')"><i>'+safe(m.glyph)+'</i><span>'+safe(p.name)+'</span></button>'}).join('');
  html+='</div>';
...
function smmOrderForm(){
  ...
  if(sections.length){
    ...
    sectionSelect='<div class="smm-field"><label>التطبيق</label><div class="smm-static">'+safe(smm.platform.name)+'</div>'+sectionTrail+'</div>';
  }
  ...
  let detail='';
  if(p){
    const desc=smmDescription(p);
    const descHtml=desc?'<div class="smm-field"><label>وصف</label><div class="smm-description">'+safe(desc).replace(/\n/g,'<br>')+'</div></div>':'';
    ...
    detail=descHtml+fieldsHtml+qtyHtml+
