/* UCHIHA Debt Store v1.5.1 — navigation, artwork and Sham Cash modal. */
(function(){'use strict';
const VERSION='1.5.1';
...
function bottom(active='categories'){
  return '<nav class="digital-bottom">'+
    '<button onclick="DigitalStore.exit()"><b>⌂</b><span>الديون</span></button>'+
    ...
    '<button class="'+(topupOpen?'active':'')+'" onclick="DigitalStore.balance()"><b>＋</b><span>إضافة رصيد</span></button></nav>';
}
...
function storeScreen(){
  const root='<div class="digital-store"><div class="digital-wrap">'+header()+statusStrip()+hero()+searchBox()+
    grid()+
    '</div>'+fab()+bottom('categories')+drawerMarkup()+topupModal()+loadingOverlay()+'</div>';
