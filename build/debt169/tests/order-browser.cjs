...
(async()=>{
  ...
  await page.addInitScript(()=>{
    ...
    const products=[
      {id:1,name:'فري فاير روبوت 1',description:'110 جوهرة',price:.955,automatic:true},
    ...
    window.Android={
      ...
      serviceRequest:(action,args,id)=>{
        ...
        setTimeout(()=>window.onDebtServiceResult(id,result),10);
      }
    };
  });
  try{
    ...
    await page.locator('.digital-card').nth(0).click();await page.waitForSelector('.digital-order-modal');
    ...
    fs.writeFileSync(path.join(output,'results.json'),JSON.stringify({pass:true,checks:['dialog overlays existing catalog','original bottom navigation preserved','fixed/amount/specificPackage/allow_quantity rules','English quantity digits','DOM-local field and price updates','conditional Free Fire verification','verification invalidation','required dynamic fields','legacy base64 custom_fields','Android back closes dialog','360/390/768 no horizontal overflow']},null,2));
