/* UCHIHA Debt Store v1.5.19 — schema-driven production order dialog. */
(function(){'use strict';
...
function smmScreen(){
  const root='<div class="digital-store smm-store smm-ref-shell">'+smmTopbar()+'<div class="digital-wrap smm-ref-wrap">'+
    ...
    '</div>'+bottom()+drawerMarkup()+topupModal()+loadingOverlay()+'</div>';
  smmCommit(root);
}
...
function grid(){
  ...
  if(products.length){
    return '<div class="digital-grid">'+products.map(p=>'<button class="digital-card" onclick="DigitalStore.product('+Number(p.id||0)+')"><div class="digital-card-media">'+imgMarkup(p,nearestPathImage())+'</div><div class="digital-card-name">'+safe(nameOf(p))+'</div><div class="digital-card-price">'+safe(money(priceOf(p)))+'</div></button>').join('')+'</div>';
  }
...
function storeScreen(){
  const root='<div class="digital-store"><div class="digital-wrap">'+header()+statusStrip()+hero()+searchBox()+
    grid()+
    '</div>'+fab()+bottom()+drawerMarkup()+topupModal()+productDialog()+loadingOverlay()+'</div>';
...
function productSubtitle(p){for(const key of ['package_name','quantity_label','short_description','description','desc']){const text=plainProductText(p?.[key]);if(text)return text.slice(0,90)}return''}
...
function orderHeadline(p,q){const subtitle=productSubtitle(p);if(subtitle)return subtitle;if(q.variable)return 'الكمية '+orderQuantity();return 'تفاصيل الطلب'}
function productDialog(){
  if(!selectedProduct)return'';
  const p=selectedProduct,fields=normalizeFields(p),q=quantityMeta(p),src=imageOf(p),automatic=selectedSchema?.automatic;
  const note=automatic===false?'يتم تنفيذ هذا المنتج يدويًا':automatic===true?'هذا المنتج يعمل بشكل تلقائي 24 ساعة':'يتم تنفيذ الطلب حسب حالة المزود';
...
function productScreen(){storeScreen()}
...
window.DigitalStore={
  async open(){
    ...
    mode='store';path=[];query='';catalog={categories:[],products:[]};lastCatalogError='';drawerOpen=false;loading=false;selectedProduct=null;selectedSchema=null;orderDraft={quantity:1,fields:{},verifiedValue:'',playerName:''};
  ...
  async smmService(id){
    ...
    const requestState=smm;
    const r=await call('digital_product',{product_id:Number(id)});if(mode!=='smm'||smm!==requestState)return;loading=false;
    if(!r.ok){smm.error=errorText(r.error);smmScreen();return;}
  ...
  closeProduct(event){if(event&&event.target!==event.currentTarget)return;selectedProduct=null;selectedSchema=null;orderDraft={quantity:1,fields:{},verifiedValue:'',playerName:''};mode='store';storeScreen()},
  ...
  debtNav(v){topupOpen=false;proofData='';proofName='';selectedProduct=null;selectedSchema=null;orderDraft={quantity:1,fields:{},verifiedValue:'',playerName:''};path=[];query='';mode='';try{nav(v)}catch(_e){render()}},
  ...
  exit(){topupOpen=false;proofData='';proofName='';selectedProduct=null;selectedSchema=null;orderDraft={quantity:1,fields:{},verifiedValue:'',playerName:''};path=[];query='';smm.product=null;smm.platform=null;smm.drop='';smm.dropSearch='';mode='';render()}
};
